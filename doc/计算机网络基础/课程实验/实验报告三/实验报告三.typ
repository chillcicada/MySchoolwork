...
+ 结果与分析：
  + 安装注意事项进行配置时，`ping` 测试正常。 下图为 HUB 设备的级联。
    #image("./assets/IMG_5597.JPG")
  + 下图为命令行共享文件夹操作及关闭共享操作。
    #image("./assets/IMG_5601.JPG")
  + 文件传输测试速率结果如下图所示，传输带宽在 10MB/s 左右。
    #image("./assets/IMG_5599.JPG")
...
+ 结果与分析：
  ...
  + 文件传输测试速率结果如下图所示，传输带宽在 100MB/s 左右
    #image("./assets/IMG_5604.JPG")
  + 下图为交换器配置界面和计算机启用 DHCP 后返回的信息。
    #image("./assets/IMG_5602.JPG")
    #image("./assets/IMG_5603.JPG")
...
- 正线和反线
  ...
  - 不同设备之间网线的选用，使用如下规则：
    #table(columns: (auto, auto), [网线的用途], [网线的类型], [HUB 普通口 $<-->$ HUB 普通口 \
      计算机 $<-->$ 计算机 / HUB Uplink 口], [反线], [HUB 普通口 $<-->$ HUB Uplink 口 \
      计算机 $<-->$ HUB 普通口], [正线])
    连接交换机时，不用考虑正反线的问题
