...
+ 实验过程：
  - 创建、更名和删除本地用户账号
    ...
    + 检查权限是否生效，执行 `net user test`，此时可看到 test 位于本地组成员 \*Administrators 和 \*Users 中；也可执行
      `net localgroup administrators`，可看到 test 为其中的成员
  - 添加用户组 guestGroup
+ 结果与分析：
+ 思考题：
  + 如何将一个帐户加入到一个组？
    - 命令行添加 `net {groupName} {userName} /add` 可将特定用户加到选定组中
    - GUI 页面添加
...
+ 实验过程：
  + 共享文件夹的设置与管理。
    ...
    - GUI 界面操作： 找到要共享的文件夹，右击属性，在属性窗口的共享选项中设置“共享”和“高级共享”
    - 命令行操作： 共享 D 盘下面的 tools 文件夹：键入 `net share tools=D:\tools`。如需删除此共享文件夹，键入 `net
       share tools /delete`。如需查看哪些文件夹在本地计算机上共享，键入 `net share`。
  + 访问共享文件夹。（计算机需要位于同一网络环境中）
    - 方法一：在“开始”-“运行”（或 win + r） 中键入 `\\计算机名\共享名`，如 `\\192.168.20.217\tools`，然后按 Enter
      键。
    - 方法二：在“我的电脑”中，选择“工具”-“映射网络驱动器”，在“映射网络驱动器”对话框中，选择“驱动器”和“文件夹”，然后单击“完成”。
    - 方法三：打开“开始”-“计算机”，在出现的窗口的地址蓝输入被访问的计算机名
    - 方法四：在命令行界面输入 `net use 映射盘盘符 \\计算机名\共享名 密码 用户名`，如 `net use Z:
       \\192.168.20.217\tools "1-217" /user:test`
+ 结果与分析：
  + 通过以上方法，可以在局域网中实现资源共享（文件、打印机），掌握资源服务器和客户端的配置方法。
+ 思考题：
  - 什么是共享文件夹？ 将存储在本地计算机上的文件夹共享，以让网络中的其他用户能够访问，这种文件夹叫共享文件夹。
    共享文件夹的优点是方便、便捷；和其他存储介质（软盘、光盘）相比，不受文件数量的大小限制。
  - 共享权限和安全权限 共享的权限只能控制网络访问，不能控制本机访问，任何适合分区，但权限的种类少。NTFS
    安全权限对本机和网络访问都能控制，权限种类多，适合精确控制。
...
+ 实验过程：
  + 关闭 139 端口
    - GUI 窗口操作
      + 打开任务管理器，切换至服务选项卡，找到并停止服务 `TCP/IP NetBIOS Helper` (对于 WIN XP 前的系统，右击本地连接-属性，点击
        internet 协议/(TCP/IP)，点击属性，找到 NETBIOS，把默认改到 *禁用* TCP/IP 上的 NETBIOS 协议，确认即可)
    - 命令行
  ...
  + 关闭 3389 端口
    ...
    - 命令行
      + 执行 `netstat -ano | findstr 3389` 查看 3389 端口占用情况，并记录 PID，而后用 `taskkill /pid {PID}
         /f` 杀死对应进程
  + 远程桌面连接
    - GUI 窗口操作
...
+ 结果与分析：
  + 139 端口是一种 TCP 端口，该端口在通过网上邻居访问局域网中的共享文件或共享打印机时就能发挥作用。该端口基于 NBT(NetBIOS over
    TCP/IP) 协议。默认的进程 PID 为 4
  + 445 端口也是一种 TCP 端口，该端口也在你通过网络访问局域网中的共享文件或共享打印机时发挥作用。但该端口基于 SMB(Server Message
    Block)Windows 协议族，用于文件和打印共享服务。
  + 3389
    端口是远程桌面的服务端口，通过这个端口，用“远程桌面”等连接工具来连接到远程的服务器，如果连接上了，输入系统管理员的用户名和密码后，将变得可以像操作本机一样操作远程的电脑，因此远程服务器一般都将这个端口修改数值或者关闭。
  + `netstat` 命令
  + `tasklist` 命令，用于查看进程，通常与 管道符 `|` 和 `findstr` 命令一起使用 使用 `tatlist` 的标准输出为： ```txt
...
- 本次实验学习到了 Windows 环境下用户配置和管理的方法，以及 Windows 环境下 NTFS
  格式文件系统的权限管理，还学习到了在局域网中实现资源共享（文件、打印机），掌握资源服务器和客户端的配置方法。最后还学习到了 TCP 端口
  139、445、3389 的作用，以及如何关闭这些端口和进程。
