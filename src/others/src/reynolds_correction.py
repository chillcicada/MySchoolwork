...
def OpenFile():
  ...
  label_path.destroy()
  label_path = tk.Label(frame_left_3, text=file_name, height=2)
  ...
  entry_end2.destroy()
  entry_end2 = tk.Spinbox(
    frame_right_2_middle, textvariable=end2, command=input_end2, from_=0, to=len_csv - 1, increment=1
  )
  entry_end2.pack(side='left', fill='both', expand=True)
  entry_start2.destroy()
  entry_start2 = tk.Spinbox(
    frame_right_2_left, textvariable=start2, command=input_start2, from_=0, to=len_csv - 1, increment=1
  )
  entry_start2.pack(side='left', fill='both', expand=True)
  entry_end1.destroy()
  entry_end1 = tk.Spinbox(
    frame_right_1_middle,
    textvariable=end1,
    command=input_end1,
    validate='all',
    validatecommand=input_end1,
    from_=0,
    to=len_csv - 1,
    increment=1,
  )
  ...
  entry_start1 = tk.Spinbox(
    frame_right_1_left, textvariable=start1, command=input_start1, from_=0, to=len_csv - 1, increment=1
  )
...
def SaveFile():
  ...
  msgbox.showinfo(title='File Saved', message='{}.png saved to {}.'.format(file_name, file_path))
  return
...
def calculate_regression():
  ...
  text_result_1 = 'Linear Fit 1: y = ({:.6} ± {:.3})x + ({:.6} ± {:.3}), r-square = {:.6f}\n'.format(
    k1, stddev_k1, b1, stddev_b1, r_square1
  )
  text_result_2 = 'Linear Fit 2: y = ({:.6} ± {:.3})x + ({:.6} ± {:.3}), r-square = {:.6f}\n'.format(
    k2, stddev_k2, b2, stddev_b2, r_square2
  )
...
def input_start1():
  ...
  """
    ...
    if Start1 >= End1:
        start1.set(End1 - 1)
        entry_start1 = tk.Spinbox(frame_right_1_left, textvariable = start1, validate = "all", validatecommand = input_start1, from_ = 0, to = end1.get() - 1, increment = 1)
        entry_start1.pack(side = LEFT, fill = BOTH, expand = True)
        return False
    elif Start1 < 0:
        start1.set(0)
  ...
  if Start1 >= 0 and Start1 < End1:
    ...
    msgbox.showinfo(
      title='Warning', message='Required: 0 <= Start 1 < End 1 < Start 2 < End 2 <= max(id)\nYou can click Remake.\n'
    )
    return False
...
def input_end1():
  ...
  """
    ...
    if End1 <= Start1:
        end1.set(Start1 + 1)
        entry_end1 = tk.Spinbox(frame_right_1_middle, textvariable = end1, validate = "all", validatecommand = input_end1, from_ = start1.get() + 1, to = start2.get() - 1, increment = 1)
        entry_end1.pack(side = LEFT, fill = BOTH, expand = True)
        return False
    elif End1 >= Start2:
        end1.set(Start2 - 1)
...
def input_start2():
  ...
  """
    ...
    if Start2 <= End1:
        start2.set(End1 + 1)
        entry_start2 = tk.Spinbox(frame_right_2_left, textvariable = start2, validate = "all", validatecommand = input_start2, from_ = end1.get() + 1, to = end2.get() - 1, increment = 1)
        entry_start2.pack(side = LEFT, fill = BOTH, expand = True)
        return False
    elif Start2 >= End2:
        start2.set(End2 - 1)
...
def input_end2():
  ...
  """
    ...
    if End2 <= Start2:
        end2.set(Start2 + 1)
        entry_end2 = tk.Spinbox(frame_right_2_middle, textvariable = end2, validate = "all", validatecommand = input_end2, from_ = start2.get() + 1, to = len_csv - 1, increment = 1)
        entry_end2.pack(side = LEFT, fill = BOTH, expand = True)
        return False
    elif End2 >= len_csv:
        end2.set(len_csv - 1)
...
def wrong_plot(csv, Start1, End1, Start2, End2, dx=0.05):
  ...
  f.set_ylabel('$\Delta T$ (K)')
  f.text(
    np.max(csv, 0)[0] * 0.2 + np.min(csv, 0)[0] * 0.8,
    np.max(csv, 0)[1] * 0.5 + np.min(csv, 0)[1] * 0.5,
    s='Required: 0 <= Start 1 < End 1 < Start 2 < End 2 <= max(id)\nYou can click Remake.\n',
    color='red',
  )  # remove first # to display warning
  f.legend()
...
def main():
  ...
  # set variables
  start1 = tk.StringVar(master=root)
  end1 = tk.StringVar(master=root)
  start2 = tk.StringVar(master=root)
  end2 = tk.StringVar(master=root)
  end2.set('3')
  start2.set('2')
  end1.set('1')
  start1.set('0')
  # root/frame_left
  frame_left = tk.Frame(root, borderwidth=5, relief='ridge')
  frame_left.pack(side='left', fill='both', expand='YES')
  # root/frame_left/frame_left_1
  frame_left_1 = tk.Frame(frame_left, borderwidth=2)
  frame_left_1.pack(fill='both')
  # root/frame_left/frame_left_1/button_file
  button_file = tk.Button(frame_left_1, text='File (*.csv, *.txt)', command=OpenFile, height=2, width=20)
  button_file.pack(fill='both')
  # root/frame_left/frame_left_2
  frame_left_2 = tk.Frame(frame_left, borderwidth=2)
  frame_left_2.pack(fill='both')
  # root/frame_left/frame_left_2/button_save
  global button_save
  button_save = tk.Button(frame_left_2, text='Save (*.png)', command=SaveFile, height=2, width=20)
  button_save.config(state='disabled')
  button_save.pack(fill='both')
  # root/frame_left/frame_left_3
  global frame_left_3
  frame_left_3 = tk.Frame(frame_left, borderwidth=5)
  frame_left_3.pack(fill='both', expand=True)
  # root/frame_left/frame_left_3/frame_left_3_1
  frame_left_3_1 = tk.Frame(frame_left_3)
  frame_left_3_1.pack(fill='both', expand=True)
  # root/frame_left/frame_left_3/frame_left_3_1/scrollbar_csv
  scrollbar_csv = tk.Scrollbar(frame_left_3_1)
  scrollbar_csv.pack(side='right', fill='y')
  # root/frame_left/frame_left_3/frame_left_3_1/listbox_csv
  global listbox_csv
  listbox_csv = tk.Listbox(frame_left_3_1, yscrollcommand=scrollbar_csv.set, selectmode='extended')
  ...
  frame_right_2_right = tk.Frame(frame_right_2)
  ...
  button_integrate = tk.Button(frame_right_2_right, text='Integrate', command=calculate_integrate, height=2, width=20)
  button_integrate.config(state='disabled')
  button_integrate.pack()
  # root/frame_right/frame_right_3
  frame_right_3 = tk.Frame(frame_right, borderwidth=5, relief='sunken')
  frame_right_3.pack(fill='both', expand=True)
  # root/frame_right/frame_right_3/text_result
  global text_result
  text_result = tk.Text(frame_right_3, height=7, wrap='char', padx=8, pady=8)
  text_result.insert('1.0', '使用说明\n')
  text_result.insert('2.0', '1. 点击File导入文件。\n')
  text_result.insert(
    '3.0', '2. csv或txt文件格式：两列纯数据，第一列为升序的time(s)，第二列Delta_T(K)；同一行数据间以半角逗号分隔。\n'
  )
  text_result.insert(
    '4.0', '3. 调整Start 1 < End 1 < Start 2 < End 2至合适位置，推荐单击输入栏后使用键盘↑↓进行调节，报错可以Remake。\n'
  )
