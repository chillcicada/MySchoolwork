...
start1, end1, start2, end2 = (
    ...
    None,
    None,
)  # 0 <= start1 < end1 < start2 < end2 < len(csv)
len_csv = 4
...
def SaveFile():
    ...
    msgbox.showinfo(
        title='File Saved', message='{}.png saved to {}.'.format(file_name, file_path)
    )
    return
...
def input_start1():
    ...
    if Start1 >= 0 and Start1 < End1:
        ...
        text_result.delete('1.0', '7.0')
        text_result.insert(
            '1.0', 'Required: 0 <= Start 1 < End 1 < Start 2 < End 2 <= max(id)\n'
        )
        ...
        msgbox.showinfo(
            title='Warning',
            message='Required: 0 <= Start 1 < End 1 < Start 2 < End 2 <= max(id)\nYou can click Remake.\n',
        )
        return False
...
def linear_regression(csv, Start, End):
    ...
    stddev_b = sr * np.sqrt(
        1 / ((End - Start) - np.sum(x) ** 2 / np.sum(np.power(x, 2)))
    )
...
def wrong_plot(csv, Start1, End1, Start2, End2, dx=0.05):
    ...
    f.plot(
        x_curve, y_curve, linewidth=1, color='#1F77B4', label='$\Delta T$ - $t$ curve'
    )
    # other plot settings
    f.set_xlabel('$t$ (s)')
    f.set_ylabel('$\Delta T$ (K)')
...
def main():
    ...
    button_file = tk.Button(
        frame_left_1, text='File (*.csv, *.txt)', command=OpenFile, height=2, width=20
    )
    ...
    button_save = tk.Button(
        frame_left_2, text='Save (*.png)', command=SaveFile, height=2, width=20
    )
    button_save.config(state='disabled')
    ...
    global listbox_csv
    listbox_csv = tk.Listbox(
        frame_left_3_1, yscrollcommand=scrollbar_csv.set, selectmode='extended'
    )
    ...
    button_remake = tk.Button(
        frame_right_1_right, text='Remake', command=remakefile, height=2, width=20
    )
