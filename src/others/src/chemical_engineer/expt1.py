...
def expt1(rho: float, mu: float) -> pd.DataFrame:
  ...
  logging.info(f'Intercept: {model[0]:.4f}, Slope: {model[1]:.4f}, R-squared: {r_squared:.4f}')
...
def expt4(rho: float, mu: float) -> pd.DataFrame:
  ...
  logging.info(f'Intercept: {model_H[0]:.4f}, Slope: {model_H[1]:.4f}, R-squared: {R_squared_H:.4f}')

  model_eta = np.polyfit(Q, eta_slash, 1)  # linear regression
  R_squared_eta = np.corrcoef(Q, eta_slash)[0, 1] ** 2
  logging.info('Fitting eta vs Q:')
  logging.info(f'Intercept: {model_eta[0]:.4f}, Slope: {model_eta[1]:.4f}, R-squared: {R_squared_eta:.4f}')
