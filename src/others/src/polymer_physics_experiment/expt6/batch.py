...
def process_chain_folder(chain_dir, output_dir):
    ...
    for idx, xml_file in enumerate(xml_files):
        try:
            ...
            box_size = [
                float(box_elem.get('lx')),
                float(box_elem.get('ly')),
                float(box_elem.get('lz')),
            ]

            # 解析粒子坐标
            pos_elem = root.find('.//position')
            pos_num = int(pos_elem.get('num'))
            positions = np.loadtxt(pos_elem.text.split(), dtype=float).reshape(
                (pos_num, 3)
            )[1:]
    ...
    plt.plot(
        time_steps,
        R_values,
        label=f'Mean Square End-to-End Distance, Variance: {R_values_variance:.4f}',
    )
    plt.plot(
        time_steps,
        S_values,
        label=f'Mean Square Radius of Gyration, Variance: {S_values_variance:.4f}',
    )
    plt.plot(
        time_steps,
        Avg_bd_values,
        label=f'Average Bond Length, Variance: {Avg_bd_values_variance:.4f}',
    )
