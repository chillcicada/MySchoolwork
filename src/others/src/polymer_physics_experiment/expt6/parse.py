...
def restore_subchain(
    sub_coords: np.ndarray, box_size, r_cut=0, initial_shift=None
    ...
    shift = (
        initial_shift.copy() if initial_shift is not None else np.zeros(3, dtype=int)
    )
...
def parallel_restore(coords: np.ndarray, box_size, r_cut=0, n_workers=4):
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        futures = [
            executor.submit(restore_subchain, sub, box_size, r_cut) for sub in subchains
        ]
...
class ChainRestore:
    ...
    def restore_subchain(
        self, sub_coords: np.ndarray, r_cut=0, initial_shift=None
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
        处理子链并返回调整后的坐标及累计平移次数
